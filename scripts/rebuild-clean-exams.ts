...
function sanitize(str: string | undefined | null): string {
  if (!str) return '';
  return str.replace(/\0/g, '').replace(/[\x00]/g, '').trim();
}

function parseSectionSmart(startLine: number, endLine: number, subjectName: string): CleanQuestion[] {
  ...
  for (let i = 0; i < rawQuestions.length; i++) {
    ...
    if (q.text.includes('Perhatikan ayat al Quran') && (q.text.includes('rel') || optA.includes('rel'))) {
      q.text = 'Perhatikan ayat al Quran (QS. Al-Hadid: 25 tentang penciptaan dan sifat besi) serta ilustrasi rel kereta api di siang hari:\n"Dan Kami ciptakan besi yang padanya terdapat kekuatan yang hebat dan berbagai manfaat bagi manusia..."\n\nBerdasarkan informasi ayat dan konsep pemuaian zat padat, maka pernyataan berikut yang benar adalah...';
      optA = 'Rel terbuat dari baja dan relnya melengkung akibat penguapan dari pemanasan di siang hari';
      optB = 'Rel terbuat dari besi dan relnya melengkung akibat pengusutan dari pendinginan di malam hari';
      optC = 'Rel terbuat dari besi dan relnya melengkung akibat pemuaian dari pemanasan di siang hari';
      optD = 'Rel terbuat dari baja dan relnya melengkung akibat beban berat kereta api di siang hari';
    }
...
async function rebuild() {
  console.log('🔄 Memperbarui butir-butir soal di Supabase dengan opsi jawaban asli...');
  ...
  const officialPackages = [
    {
      ...
      description: 'Mata Uji Resmi Keislaman (Fikih, Quran Hadits, SKI, dan Akidah Akhlak). Murni soal materi agama Islam.',
      startLine: 1323,
      endLine: 1497,
      durationMinutes: 45,
    },
    {
      ...
      token: 'IC-ARAB',
      subject: 'Bahasa Arab',
      description: 'Mata Uji Resmi Bahasa Arab (Qira\'ah, Tarkib, Mufradat, dan Fahmul Masmu\').',
      startLine: 13,
      endLine: 131,
      durationMinutes: 45,
    },
    {
      ...
      description: 'Mata Uji Resmi Bahasa Indonesia (Ide Pokok, Kalimat Efektif, Ejaan, dan Pemahaman Teks).',
      startLine: 131,
      endLine: 200,
      durationMinutes: 45,
    },
    {
      ...
      description: 'Mata Uji Resmi Bahasa Inggris (Reading Comprehension, Vocabulary in Context, and Text Structure).',
      startLine: 200,
      endLine: 308,
      durationMinutes: 45,
    },
    {
      ...
      token: 'IC-MAT',
      subject: 'Matematika',
      description: 'Mata Uji Resmi Matematika Terpadu (Aljabar, Geometri, Peluang, dan Aritmetika Sosial).',
      startLine: 1497,
      endLine: 1620,
      durationMinutes: 45,
    },
    {
      ...
      description: 'Mata Uji Resmi IPA Terpadu (Fisika, Biologi, dan Kimia Terapan).',
      startLine: 1126,
      endLine: 1228,
      durationMinutes: 45,
    },
    {
      ...
      description: 'Mata Uji Resmi IPS Terpadu (Sejarah, Geografi, Ekonomi, dan Sosiologi).',
      startLine: 1228,
      endLine: 1323,
      durationMinutes: 45,
    },
    {
      ...
      description: 'Mata Uji Resmi Kemampuan Analitik, Silogisme, Analogi, dan Pemecahan Masalah Logika.',
      startLine: 7248,
      endLine: 7623,
      durationMinutes: 45,
    },
    {
      ...
      description: 'Paket Komprehensif Tes Akademik (TA) MAN Insan Cendekia Paket 1 lengkap mencakup seluruh subtes integrasi.',
      startLine: 308,
      endLine: 1126,
      durationMinutes: 120,
    },
    {
      ...
      description: 'Paket Komprehensif Tes Akademik (TA) MAN Program Keagamaan Paket 1.',
      startLine: 1620,
      endLine: 2270,
      durationMinutes: 90,
    },
  ...
  for (const pkg of officialPackages) {
    const questions = parseSectionSmart(pkg.startLine, pkg.endLine, pkg.subject);

    const exists = await prisma.exam.findUnique({ where: { token: pkg.token } });
    if (exists) {
      await prisma.answerSubmission.deleteMany({ where: { session: { examId: exists.id } } });
    ...
    const dummyCount = questions.filter(q => q.optionA.includes('Pilihan A')).length;
    console.log(`✅ [SELESAI] ${exam.title} | Token: ${exam.token} | ${questions.length} Soal | Opsi Dummy: ${dummyCount}`);
  }

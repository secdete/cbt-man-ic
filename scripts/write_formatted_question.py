...
export default function FormattedQuestionText({
  ...
  return (
    <div dir={isArabic ? 'rtl' : 'auto'} className={`${arabicStyle} ${className}`}>
      {elements}
    </div>
  );
}
...
with open('components/FormattedQuestionText.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Saved components/FormattedQuestionText.tsx successfully!")

...
export async function extractTextFromPDF(buffer: Buffer): Promise<string> {
  try {
    ...
    throw new Error(`Ekstraksi PDF gagal: ${error?.message || 'Error tidak diketahui'}`);
  }
}
...
export function parseQuestionsFromText(rawText: string): ParsedQuestion[] {
  ...
  // Bersihkan karakter aneh dan standarisasi baris baru
  ...
  const keySectionRegex = /(?:kunci\s*jawaban|daftar\s*kunci|kunci\s*soal)[\s\S]*$/i;
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const endIndex = i + 1 < matches.length ? matches[i + 1].index : (keyMatch ? keyMatch.index : cleanText.length);
  ...
  for (const block of questionBlocks) {
    const parsed = parseSingleQuestionBlock(block.number, block.text, bottomKeysMap.get(block.number));
    if (parsed) {
      results.push(parsed);
    }
  }
...
function parseSingleQuestionBlock(
  ...
  let explanation = '';
  const explMatch = mainText.match(/(?:pembahasan|penjelasan|alasan)\s*[\:\-]\s*([\s\S]+)$/i);
  if (explMatch) {
  ...
  let inlineKey = '';
  const keyMatch = mainText.match(/(?:kunci|jawaban|kunci\s*jawaban)\s*[\:\-]\s*([A-Ea-e])/i);
  if (keyMatch) {
  ...
  if (optMatches.length >= 2) {
    ...
    for (let j = 0; j < optMatches.length; j++) {
      ...
      const nextIndex = j + 1 < optMatches.length ? optMatches[j + 1].index : mainText.length;
      const optContent = mainText.slice(cur.index + cur.matchLength, nextIndex).trim();

      switch (cur.letter) {
        case 'A': optA = optContent; break;
        case 'B': optB = optContent; break;
        case 'C': optC = optContent; break;
        case 'D': optD = optContent; break;
  ...
  return {
    ...
    correctAnswer: ['A', 'B', 'C', 'D', 'E'].includes(finalKey) ? finalKey : 'A',
...
function detectSubject(text: string): string {
  ...
  if (t.includes('hitung') || t.includes('matematika') || t.includes('segitiga') || t.includes('fungsi') || t.includes('x =') || t.includes('bilangan') || t.includes('rata-rata') || t.includes('peluang')) {
    return 'Penalaran Matematika';
  }
  if (t.includes('surah') || t.includes('ayat') || t.includes('hadis') || t.includes('zakat') || t.includes('sholat') || t.includes('nabi') || t.includes('islam') || t.includes('fikih') || t.includes('akidah')) {
    return 'Literasi Keagamaan';
  }
  if (t.includes('paragraf') || t.includes('ide pokok') || t.includes('kalimat utama') || t.includes('bacaan') || t.includes('kesimpulan') || t.includes('simpulan')) {
    return 'Literasi Membaca';
  }
  if (t.includes('which') || t.includes('the following') || t.includes('passage') || t.includes('according to') || t.includes('the author')) {
    return 'Literasi Bahasa Inggris';
  }
  if (t.includes('jika semua') || t.includes('maka kesimpulannya') || t.includes('urutan') || t.includes('analogi') || t.includes('pola')) {
